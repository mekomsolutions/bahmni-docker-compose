...
ODOO_BACKUP_FILENAME=${ODOO_DB_FILENAME:-odoo.tar}
ODOO_BACKUP_FILE=/opt/restore/$ODOO_BACKUP_FILENAME
...
OPENELIS_DB_FILENAME=${OPENELIS_DB_FILENAME:-clinlims.tar}
OPENELIS_BACKUP_FILE=/opt/restore/$OPENELIS_DB_FILENAME
...
if [ -f "$OPENELIS_BACKUP_FILE" ]; then
	...
	PGPASSWORD=$POSTGRES_PASSWORD psql -h postgresql -U postgres -c "ALTER DATABASE ${OPENELIS_DB_NAME} OWNER TO ${OPENELIS_DB_USER};"
fi
